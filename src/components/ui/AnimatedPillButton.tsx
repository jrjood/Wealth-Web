...
const toneClasses = {
  ...
  dark: {
    ...
    button: 'bg-[hsl(var(--brand-black))] border-transparent',
    overlay: 'bg-[hsl(var(--brand-cream))]',
    primaryLabel: 'text-white',
  },
  light: {
...
const AnimatedPillButton = ({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn('relative z-[3] block overflow-hidden', innerClassName)}
      >
        <span className='relative block'>
          <span
            className={cn(
              'block text-center font-sans font-semibold transition-transform duration-700 [transition-timing-function:cubic-bezier(0.77,0,0.175,1)] group-hover:translate-y-[150%]',
          ...
          <span
            aria-hidden='true'
            className={cn(
              'absolute left-1/2 top-[-150%] block w-full -translate-x-1/2 -translate-y-1/2 text-center font-sans font-semibold transition-[top] duration-700 [transition-timing-function:cubic-bezier(0.77,0,0.175,1)] group-hover:top-1/2',
